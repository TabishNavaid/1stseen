...
| Motion | `lift`, `press`, `rise`, `fade-in`, `drop-in`, `drift-track`, `bob-once`, `step-in`, `pop-in`, `stamp-in` | A hovered card, a pressed button, hero text entering, the hero chart's openings dropping in and its window fading in, the Just opened strip drifting, every hand-drawn character's single bob as it arrives (`--bob` sets how far, 8px by default and 4px on a small one), a first-run step entering, the payoff's character arriving, and the stamp landing on a save. Every one stops under `prefers-reduced-motion`, where the stamp is simply already there, and the strip does not move |
...
| Brand mark | `BrandMark` in `brand-mark.tsx`, drawing `BirdMark` in `brand/bird-mark.tsx` | The bird on its own tile, then the wordmark. The drawing is `design-refs/icon` as it stands (`docs/credits.md`), written out by `scripts/build-brand-art.mjs`, and it also makes `favicon.svg`, `icon-32.png` and `apple-touch-icon.png`. `og-image.jpg` is a photograph of the live landing page, which carries the mark because the header does. It clips itself, and a clip is named, so each copy on a page takes its own `markId` |
| Cameo | `Bird` in `brand/bird.tsx`, sizes `perch` (56 to 72px), `cameo` (48 to 64px), `tiny` (40 to 44px) | The bird beside something, never instead of it: the corner of the chart card, the save stamp, a search that found nothing, the head of a digest. One to a screen, and never where it would take a moment from the person in it. Decoration unless given a `label` |
