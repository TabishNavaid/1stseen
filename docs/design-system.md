...
| Brand mark | `BrandMark` and `BrandLockup` in `brand-mark.tsx`, drawing `BirdMark` in `brand/bird-mark.tsx` | The wordmark with the bird's head. `LOCKUP` at the top of `brand-mark.tsx` picks one arrangement for the whole product: `tile`, `perched` or `beside`. The same drawing makes `favicon.svg`, `icon-32.png`, `apple-touch-icon.png` and the corner of `og-image.png` |
