# How 1stSeen is built

The engineering overview that used to be the repository's README: the product contract, the forecast engine, collection,
the data model, and the commands behind each part. The [docs index](README.md) lists every deeper document.
...
documented in [the forecasting methodology](forecasting-methodology.md).
...
[the backtesting methodology](backtesting-methodology.md).
...
[the recruiting signal methodology](recruiting-signals.md).
...
than degrading to a fixture. See [the agent architecture](recruiting-agent.md).
...
[`docs/watchlists-and-personalization.md`](watchlists-and-personalization.md).
...
step fails are in [`docs/account-data.md`](account-data.md).
...
Supabase; details in [`docs/local-development.md`](local-development.md)):
...
([`docs/operations.md`](operations.md)). All collection jobs
...
[`docs/github-actions-collection.md`](github-actions-collection.md).
...
documented manual step. Collection can honour robots.txt before every request (`ROBOTS_TXT_ENFORCED`, off by default in code and on in production).
The procedure, timelines, and the measured cost of turning robots.txt on are in [`docs/takedown.md`](takedown.md).
...
in [`docs/scraping-reliability-audit.md`](scraping-reliability-audit.md).
The ML methodology review, corrected leakage paths, and remaining calibration limitations are documented in
[`docs/forecasting-audit.md`](forecasting-audit.md).
...
- `google_calendar_connections` holds server-only encrypted OAuth credentials; `calendar_event_syncs`
  ...
  semantics are documented in [`docs/google-calendar-integration.md`](google-calendar-integration.md).
- `gmail_connections` stores separately consented, encrypted Gmail credentials. `email_digest_deliveries`
  ...
  [`docs/email-intelligence-digests.md`](email-intelligence-digests.md).
- The Application Readiness Planner works backward from every prediction interval into networking,
  ...
  [`docs/readiness-methodology.md`](readiness-methodology.md).
...
- `account_deletions` records that an account was deleted, with counts and Google revocation outcomes and no personal
  data. See [`docs/account-data.md`](account-data.md).
...
- `collection_takedowns` records every stop, resume, withdrawal, and erasure of collection: who, when, why, and which
  sources and roles changed ([`docs/takedown.md`](takedown.md)).
...
The scheduled current-jobs workflow (twice a day, `docs/operations.md`) installs only the worker package, reads enabled sources from Supabase, stores content-addressed observations, and records every adapter run as an agent tool call.
