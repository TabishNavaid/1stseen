...
import { JUST_OPENED_DAYS, JUST_OPENED_PAGE_SIZE, hasServiceRoleConfig, loadJustOpened } from "@/lib/real-data";
...
const pageHref = (page: number) => (page > 1 ? `/opened?page=${page}` : "/opened");

/** Every program that opened in the last 45 days, by the job boards' own dates, a page at a time. */
export default async function JustOpenedPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (hasServiceRoleConfig()) {
    const data = await loadJustOpened(undefined, { limit: JUST_OPENED_PAGE_SIZE, offset: (page - 1) * JUST_OPENED_PAGE_SIZE });
    ...
    total = data.total;
    mode = "real";
  } else if (process.env.FIRSTSEEN_DEMO_MODE === "true") {
  ...
  return (
    <>
      ...
      <main id="opened-content" className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 md:px-6 md:py-10">
        <p className="label-caps text-accent-ink">Posted in the last {JUST_OPENED_DAYS} days</p>
        <h1 className="heading-display mt-2 text-3xl leading-tight sm:text-4xl">Just opened</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-muted">
          {mode === "unconfigured"
            ? "Live data is not configured on this deployment, so there is nothing to list."
            : total === 0
              ? `No program has opened in the last ${JUST_OPENED_DAYS} days.`
              : `${total.toLocaleString("en-US")} ${total === 1 ? "program" : "programs"} opened in the last ${JUST_OPENED_DAYS} days, newest first. Each date is the day the company posted it.`}
        </p>

        {programs.length > 0 && <div className="mt-8"><RecruitingTimeline programs={programs} /></div>}
        {programs.length === 0 && total > 0 && (
          <p className="mt-8 text-sm text-ink-muted">This page is past the last one. <Link href="/opened" className="link-accent focus-ring">Go to the newest</Link></p>
        )}

        {total > JUST_OPENED_PAGE_SIZE && programs.length > 0 && (
          <nav className="mt-6 flex flex-wrap items-center justify-between gap-2 text-caption text-ink-subtle" aria-label="Pages">
            <span>{first.toLocaleString("en-US")}–{last.toLocaleString("en-US")} of {total.toLocaleString("en-US")}</span>
            <span className="flex items-center gap-2">
              {page > 1 && <Link href={pageHref(page - 1)} className="focus-ring inline-flex min-h-touch items-center gap-1 rounded-chip border border-line-strong bg-surface px-4 font-semibold text-ink hover:bg-surface-hover"><Icon name="arrow-left" size={13} />Newer</Link>}
              {last < total && <Link href={pageHref(page + 1)} className="focus-ring inline-flex min-h-touch items-center gap-1 rounded-chip border border-line-strong bg-surface px-4 font-semibold text-ink hover:bg-surface-hover">Older<Icon name="arrow-right" size={13} /></Link>}
            </span>
          </nav>
        )}
      </main>
