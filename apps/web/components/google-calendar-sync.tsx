...
/**
 * Adding calendar dates to Google, when this deployment has the Google credentials for it. Whether it has them is
 * settled on the server and the button is simply not drawn without them (app/calendar/page.tsx): a reader should
 * never be told what an operator has or has not configured. The status this fetches can still say otherwise, and
 * then the drawer offers nothing to connect.
 */
export function GoogleCalendarSync({ events }: { events: CalendarEvent[] }) {
  ...
  return <>
    ...
    {open && <><div className="fixed inset-0 z-40 bg-scrim" onClick={() => setOpen(false)} aria-hidden="true" /><aside ref={panelRef} role="dialog" aria-modal="true" className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[480px] flex-col border-l border-line-strong bg-surface shadow-dialog" aria-label="Google Calendar sync"><div className="flex items-start justify-between border-b border-line p-5"><div><h2 className="text-xl font-semibold tracking-title">Add dates to Google Calendar</h2><p className="mt-2 max-w-sm text-xs leading-5 text-ink-muted">Nothing is added on its own. Pick the dates you want: a step in your prep plan, or the start and end of a likely window.</p></div><Button variant="ghost" size="icon" onClick={() => setOpen(false)} aria-label="Close"><Icon name="x" size={17} /></Button></div>
      <div className="border-b border-line p-4">{!status ? <div className="flex items-center gap-2 text-xs text-ink-muted"><Icon name="loader-circle" size={14} className="animate-spin" />Checking connection…</div> : !status.connected ? status.configured === false ? null : <div><p className="text-xs leading-5 text-ink-muted">Connect your Google account before choosing events. Google will ask to let 1stSeen view and edit events on all your calendars; 1stSeen only adds, updates, and removes the events you choose here. <a href="/privacy/google#calendar" className="link-accent focus-ring">How Google data is used</a></p><a href="/api/integrations/google-calendar/connect" className="mt-3 inline-flex h-9 items-center gap-2 rounded-md bg-accent px-3 text-xs font-semibold text-ink-inverse"><Icon name="calendar-plus" size={14} />Connect Google Calendar</a></div> : <div className="flex items-center justify-between gap-3"><div><p className="flex items-center gap-2 text-xs font-semibold text-accent-ink"><span className="grid h-5 w-5 place-items-center rounded-full bg-accent-soft"><Icon name="check" size={11} /></span>Google Calendar connected</p>{status.account_label && <p className="ml-7 mt-1 text-micro text-ink-subtle">{status.account_label}</p>}</div><Badge className="border-success-line bg-success-surface text-accent-ink">{status.synced_source_keys.length} synced</Badge></div>}</div>
